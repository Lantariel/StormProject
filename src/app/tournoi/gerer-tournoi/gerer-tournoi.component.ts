...
/* === Recherche d'un joueur dans la base de donnée ==== */
  ...
  onInscrireJoueurDepuisRecherche(id: string): void {
    ...
    if (this.alreadyRegistered === false) // Si on a pas trouvé de doublon
    {
      ...
      this.tournoiService.ajouterUnJoueur(this.currentTournamentIndex, this.tempJoueur);
    }
  ...
  onDesinscrireJoueur(id: number): void {
  ...
  onClearReasearch(): void{
    this.joueursTrouves = [] ;
    this.formRecherche.reset() ;
  }

  checkIfPlayerAlreadyRegistered(pId: string): boolean{
  ...
  onAddEditor(): void{
    ...
    this.tournoiService.addEditor(this.tournoi, edit) ;
  }

  onDeleteEditor(id: number): void{
    this.tournoi.editors.splice(id, 1) ;
    this.tournoiService.removeEditor(this.tournoi, id) ;
  }

  onSetTournamentTop(): void {
  ...
  forcerNombreDeRondes(nb: number): void {
    ...
    this.tournoiService.activateFixedRoundNumber(this.tournoi) ;
  }

  onAnnulerForcageDesRondes(): void {
    this.forceRoundNumber = null ;
    this.calculerNombreDeRondes() ;
    this.tournoiService.desactivateFixedRoundNumber(this.tournoi, this.tournoi.nombreDeRondes) ;
    this.tournoi.roundNumberIsFixed = false ;
  }

  onCommencerTournoi(): void {
  ...
  onToggleFinals(): void{
    ...
    this.tournoiService.setFinalsActivation(this.tournoi, this.tournoi.finalBracket) ;
  }

  matchResearch(id: number): boolean{
  ...
  matchRegResearch(id: number): boolean{
