...
export class FinalmatchesComponent implements OnInit {
  ...
  this.tournoiService.createFinalMatches(this.tournoi) ;
  ...
  onSetScore(matchID: number): void{
    ...
    if (score1 !== score2)
    {
      ...
      this.tournoiService.enterScore(this.tournoi, matchID, score1, score2, false, false) ;
    }
    else
    {
  ...
  toogleDisplayPenaltyPannel(): void{
  ...
  matchResearch(table: number): any{
    ...
    else
    {
      ...
      {
        return this.getPlayer(this.tournoi.currentMatches[table].joueur1).firstName.toLowerCase().search(research.toLowerCase()) !== -1
          || this.getPlayer(this.tournoi.currentMatches[table].joueur1).lastName.toLowerCase().search(research.toLowerCase()) !== -1
          || this.getPlayer(this.tournoi.currentMatches[table].joueur2).firstName.toLowerCase().search(research.toLowerCase()) !== -1
          || this.getPlayer(this.tournoi.currentMatches[table].joueur2).lastName.toLowerCase().search(research.toLowerCase()) !== -1;
      }
    }
  }

  setFocusTable(id: number): void{
    this.tableFocus = id ;
    this.errorMsg = null ;
  }
  ...
  onNextStep(): void{
    this.tableFocus = -1 ;
    this.tournoiService.nextFinalStep(this.tournoi) ;
  ...
  onEndTournament(): void{
    this.tournoiService.endTournament(this.tournoi) ;
